...
long long Sum[N];
long long power[N];
inline long long Change(long long S,long long t){
    return (Sum[t]-Sum[S-1]*power[t-S+1]%mo+mo)%mo;
}
int main(){
    scanf("%I64d%I64d",&n,&k);scanf("%s",s+1);
    fac[0]=1;for(long long i=1;i<=n;i++)fac[i]=1ll*fac[i-1]*i%mo;
    ifac[0]=1;ifac[n]=Power(fac[n],mo-2);for(long long i=n-1;i;i--)ifac[i]=1ll*ifac[i+1]*(i+1)%mo;
