...
long long strhash(int i, int j)
{
    if(i == 0)
        return h[j];
    return ((h[j] - h[i - 1] * mpow[j - i + 1]) % MOD + MOD) % MOD;
}
...
bool try_2(time_t t)
{
    vector<int> rand_lol;
    for(int i = 1; i <= l; ++i)
        rand_lol.push_back(i);
    for(int i = 1; i < 10; ++i)
        random_shuffle(rand_lol.begin(), rand_lol.end());
    for(int i : rand_lol)
    {
        for(int j : rand_lol)
        {
...
int main()
{
    ...
    {
        cout << "hash " << i << " = " << h[i] << endl;
        for(int j = i; j < slen; ++j)
        {
            cout << "subhash " << i << " " << j << " = " << strhash(i, j) << endl;
        }
    }*/
