/** Micro Mezz Macro Flation -- Overheated Economy ., Last Update: May. 7th 2014 **/ //{
...
const int MOD = 1234567891; //int(1e9) + 7;
...
int G[N][N], D[N], P[N]; bool C[N];
char A[N][12][12]; LL mst;
int n, m,k,w;
...
void Mst(){
    ...
    fill(D, D+k, n*m); FLC(P, -1);
res.clear();

     REP(i, k){

        int u = 0; while (C[u]) ++u; FOR(i, u+1, k) if (!C[i] && D[i] < D[u] ) u = i;
        mst += D[u]; C[u] = 1;


    res.PB(MP(u, P[u]));
        REP(i, k) if (!C[i] && G[u][i]*w < D[i]){
            D[i] = G[u][i]*w;
            P[i] = u;
        }
    }
...
int main(){
   ...
   //printf("%I64d\n",gcd(-6,12));
    ...
    while (~scanf("%d%d%d%d", &n, &m, &k, &w)){
        REP(kk, k) REP_2(i, j, n, m) RC(A[kk][i][j]);
        RST(D); REP_2(j, i, k, j){
            G[i][j] = 0; REP_2(ii, jj, n, m){
                G[i][j] += A[i][ii][jj] != A[j][ii][jj];
            }
            G[j][i] = G[i][j];
